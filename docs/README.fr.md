...
Sur un PC configuré par l’installeur, une tâche planifiée vérifie à l’ouverture de session puis toutes les cinq minutes si ce fond est sélectionné. Elle lance le relais sans navigateur ni console, ou réutilise le relais existant. Le relais démarré automatiquement s’arrête après dix minutes sans client. Wallpaper Engine utilise son propre démarrage automatique. Pour un lancement manuel de développement :
...
- Le serveur de courrier, le parvis et les pas japonais sont des sprites générés en pixel art. Les conduits partent de la bouche du serveur, dont les voyants suivent l’activité. Robots et chien laissent des empreintes qui disparaissent après huit secondes (128 maximum). [Assets et fonctionnement](GARDEN-DETAILS.md).
- Le réglage **Fond → Cycle réel — ciel bleu** ajoute un ciel bleu de jour et bleu de minuit la nuit, en gardant le sol sombre. Soleil, nuages lents, lune, étoiles qui scintillent indépendamment et étoiles filantes à trajectoires variables suivent l’heure locale et la pause. Un muret en texture pixel art répétable délimite le jardin derrière la maison ; il ne s’étire pas sur un écran large. [Détails du cycle](SKY-CYCLE.md), [texture du muret](WALL-TEXTURE.md), [aperçu jour/nuit](https://drmoussavie.github.io/agentic-wallpaper/sky-preview.html).
- Chaque famille de robots possède dix skins stables par session : classique à 75 %, couleurs moins fréquentes, Stardust jaune doré à 1 %. Leur silhouette et leurs antennes restent identiques. [Palettes et probabilités](COSMETICS.md).
...
Cela enregistre une tâche planifiée de l’utilisateur (« Agentic Wallpaper relay ») : à l’ouverture de session puis toutes les 5 minutes, `scripts/start-relay.ps1` lit la configuration de Wallpaper Engine et ne lance le relais **que si Agentic Wallpaper est sélectionné sur un écran** ; lancé ainsi (`--auto`), le relais s’arrête tout seul après 10 minutes sans fond ni Studio connecté. Changer de fond suffit donc à ne plus rien faire tourner ; le resélectionner relance le relais dans les 5 minutes. Aucun droit administrateur, aucune modification permanente de la stratégie PowerShell, aucun démarrage supplémentaire de Wallpaper Engine. Le relais réutilise le processus existant et n’ouvre pas de navigateur. Journaux bornés par écrasement au démarrage dans `.local/autostart-*.log`. Pour retirer uniquement ce démarrage : même commande avec `-Remove` (l’ancien raccourci du dossier Démarrage est retiré au passage). Conserver le projet à son emplacement actuel ; refaire cette configuration après déplacement du dossier.

`Install.ps1` à la racine enchaîne tout (dépendances, hooks, export et copie dans Wallpaper Engine, tâche planifiée, premier lancement) avec un petit robot ASCII qui marche pendant les étapes ; `Uninstall.ps1` défait tout (arrêt du relais, hooks, tâche, copie du fond — options `-KeepHooks`, `-KeepWallpaper`). Un vrai installeur graphique avec les sprites animés demanderait une application à part ; ce n’est pas prévu pour l’instant.
