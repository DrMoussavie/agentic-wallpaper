(function(){
  ...
  function updateUi(){
    ...
    agents.forEach(a=>{const row=document.createElement('div');row.className='activity-row';const dot=document.createElement('i');dot.className=a.provider==='codex'?'cyan':'amber';const name=document.createElement('span');name.textContent=(a.parent?'↳ ':'')+a.name;if(a.parent)row.classList.add('subagent-row');const state=document.createElement('small');const phase=renderer.life.actors.get(a.id)?.phase;state.textContent=({home:'À la maison',returning:'Rentre à la maison',entering:'Entre à la maison',queued:'Dans la maison',leaving:'Sort de la maison'})[phase]||TransitAnimations.ACTIONS[a.action]?.name||'Veille';row.append(dot,name,state);list.append(row);});
  ...
  ['tubes','pet','roam','social','audio','media','bubbles','toy','background','audioSource','scale','bubbleScale'].forEach(id=>q(id)?.addEventListener('change',()=>{renderer.settings[id]=['tubes','pet','roam','social','audio','media','bubbles','toy'].includes(id)?q(id).checked:['scale','bubbleScale'].includes(id)?Number(q(id).value):q(id).value;syncAudio();renderer.resize();renderer.draw();}));
  ...
  canvas.addEventListener('pointerdown',e=>{
    ...
    if(renderer.pointerDown(e.clientX-b.left,e.clientY-b.top,e.timeStamp)){capturedPointer=e.pointerId;canvas.setPointerCapture(e.pointerId);canvas.style.cursor='grabbing';}
    renderer.draw();
  });
  canvas.addEventListener('pointermove',e=>{if(!e.isPrimary||paused||enginePaused)return;const b=canvas.getBoundingClientRect();canvas.style.cursor=renderer.pointerMove(e.clientX-b.left,e.clientY-b.top,e.timeStamp);});
  function releasePointer(stamp,cancel=false){renderer.pointerUp(stamp,cancel);if(capturedPointer!==null&&canvas.hasPointerCapture(capturedPointer))canvas.releasePointerCapture(capturedPointer);capturedPointer=null;canvas.style.cursor='crosshair';}
  canvas.addEventListener('pointerup',e=>{if(e.pointerId===capturedPointer)releasePointer(e.timeStamp);});
  canvas.addEventListener('pointercancel',e=>{if(e.pointerId===capturedPointer)releasePointer(e.timeStamp,true);});
  canvas.addEventListener('lostpointercapture',e=>{if(e.pointerId===capturedPointer)releasePointer(e.timeStamp,true);});
  canvas.addEventListener('pointerleave',e=>{if(capturedPointer===null)renderer.pointerUp(e.timeStamp,true);});
  window.addEventListener('blur',()=>releasePointer(performance.now(),true));
  // Must be assigned synchronously for Wallpaper Engine's initial property callbacks.
  window.wallpaperPropertyListener={
    applyUserProperties(p){if(p.audioSource)renderer.settings.audioSource=p.audioSource.value==='wallpaper-engine'?'wallpaper-engine':'relay';for(const id of ['tubes','pet','roam','social','audio','media','bubbles','toy'])if(p[id])renderer.settings[id]=!!p[id].value;if(p.pixelscale)renderer.settings.scale=Number(p.pixelscale.value)||1;if(p.bubbleScale)renderer.settings.bubbleScale=Math.max(.75,Math.min(2,Number(p.bubbleScale.value)||1.4));if(p.audioWidth)renderer.settings.audioWidth=Math.max(20,Math.min(100,Number(p.audioWidth.value)||35));if(p.bottomMargin)renderer.settings.bottomMargin=Math.max(0,Math.min(200,Number(p.bottomMargin.value)||0));if(p.background)renderer.settings.background=['black','night','grid'][Number(p.background.value)]||'black';syncAudio();renderer.resize();renderer.draw();},
