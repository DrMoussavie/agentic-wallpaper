(function(){
  ...
  ['tubes','pet','roam','social','audio','media','bubbles','toy','hoop','visitors','terrain','background','audioSource','scale','bubbleScale'].forEach(id=>q(id)?.addEventListener('change',()=>{renderer.settings[id]=['tubes','pet','roam','social','audio','media','bubbles','toy','hoop','visitors','terrain'].includes(id)?q(id).checked:['scale','bubbleScale'].includes(id)?Number(q(id).value):q(id).value;syncAudio();renderer.resize();renderer.draw();}));
