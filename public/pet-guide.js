(function(root){
  ...
  class Guide {
    ...
    update(dt,life,world,enabled,toy,chase=null){
      ...
      const b=life.bounds;
      // A butterfly is worth a chase when nothing real needs the dog.
      const chasing=!target&&!toyTarget&&chase?{x:clamp(chase.x,b.left,b.right),y:clamp(chase.y+9,b.top,b.bottom)}:null;if(chasing)this.job='chase';
      if(!target&&!toyTarget&&!chasing){
      ...
      const dx=destination.x-this.x,dy=destination.y-this.y,d=Math.hypot(dx,dy),speed=Math.max(28,Math.hypot(life.width,life.height)/(target?9:toyTarget?10:chasing?15:28)),step=Math.min(d,speed*clamp(dt,0,.2));
