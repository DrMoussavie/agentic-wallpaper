...
test('Codex et Claude : sortie, fin de réponse, retour réel à la porte, repos et reprise du même agent',()=>{
  for(const provider of ['codex','claude']){const {w,l}=setup(0);w.apply({provider,sessionId:'a',type:'session_start'});l.update(0,w);const a=l.actors.get(`${provider}:a`);assert.equal(a.phase,'queued');run(w,l,25);assert.equal(a.phase,'outside');w.apply({provider,sessionId:'a',type:'stop'});run(w,l,7);
  // The answer is delivered: the robot waits in the garden with the letter, then goes home after five minutes at most.
  assert.equal(a.phase,'outside');assert.equal(l.lingering(a),true);
  // It walks to the waiting corner, sits on the bench with the letter, and never drifts away from its seat.
  for(let i=0;i<600&&!a.parked;i++){w.update(.1);l.update(.1,w);}
  assert.equal(a.parked,true);assert.equal(a.seated,true);assert.deepEqual(l.visual(a,w),{...l.visual(a,w),action:'idle',delivered:true,seated:true});
  const seat=[a.x,a.y];run(w,l,60);assert.deepEqual([a.x,a.y],seat);assert.equal(a.phase,'outside');
  const c=l.scene.corner;assert.ok(a.x>=c.left&&a.x<=c.right&&a.y>=c.top&&a.y<=c.bottom,'assis dans le coin dessiné');
  run(w,l,240);assert.equal(a.phase,'home');assert.equal(a.spot,null);assert.equal(l.outside().length,0);assert.equal(l.stats.returned,1);assert.equal(w.agents.size,1);w.apply({provider,sessionId:'a',type:'prompt'});l.update(0,w);assert.equal(a.phase,'queued');assert.equal(a.x,l.scene.door.x);run(w,l,25);assert.equal(a.phase,'outside');assert.equal(w.agents.size,1);assert.equal(l.stats.departed,2);}
});
