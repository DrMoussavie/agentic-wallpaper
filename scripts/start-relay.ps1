...
try {
    ...
    $relay = Start-Process -FilePath $nodeExe -ArgumentList 'bridge/server.mjs' -WorkingDirectory $projectPath -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logPath 'autostart-relay.log') -RedirectStandardError (Join-Path $logPath 'autostart-relay-error.log') -PassThru
